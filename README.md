...
Важно: в текущем MVP safety/policy-контур уже покрывает не только command/report/audit/rollback flow, но и прямые мутации проекта/goal/task. `POST /api/v1/projects`, `POST /api/v1/projects/{project_id}/tasks`, `POST /api/v1/projects/{project_id}/goals` и `POST /api/v1/projects/{project_id}/tasks/{task_id}/decompose` проходят через direct-mutation policy-check, уважают safe-pause и пишут audit event на выполнение. Отдельно от них живёт `POST /api/v1/projects/onboard`: это special onboarding-service path со своим onboarding audit и bootstrap side effects, а не тот же direct-mutation gate.
...
Safe-pause уже реально работает как защитный стоп в policy engine для command/report/audit/rollback-путей и для direct-mutation project/goal/task endpoint'ов.
...
Важно: прямые CRUD endpoint'ы проектов/goal/task'ов больше не являются исключением: они тоже проходят через direct-mutation `PolicyEngine` gate и пишут audit event. Исключение здесь только `POST /api/v1/projects/onboard`, потому что это отдельный onboarding-service flow со своим audit/smoke-check поведением.
...
- для мутации нужны query-параметры `actor` и `role`, плюс header `x-actor-trusted`.
...
- это отдельный onboarding-service path, а не direct-mutation policy endpoint.
