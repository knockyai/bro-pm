...
- прогоняет smoke check через выбранную board integration; для `yandex_tracker` это live `create_task` API-вызов;
...
### Интеграции: Yandex Tracker live, остальные пока mostly stub-like
...
- `yandex_tracker` для `create_task` больше не stub: адаптер собирает live HTTP-запрос к Yandex Tracker API, валидирует runtime config, использует queue из `project.metadata.integrations.yandex_tracker.queue` с fallback на env default queue и нормализует ответ с issue key/id;
...
То есть живой внешний API-вызов сейчас реализован только для `yandex_tracker create_task`; остальные интеграции по умолчанию всё ещё MVP/stub-like.
...
- `BRO_PM_YANDEX_TRACKER_API_BASE`
  ...
  - используется live `yandex_tracker create_task` адаптером.

- `BRO_PM_YANDEX_TRACKER_TOKEN`
  - обязателен для live `yandex_tracker create_task`.
...
- `BRO_PM_YANDEX_TRACKER_DEFAULT_QUEUE`
  - optional fallback queue для live `yandex_tracker create_task`;
...
- пройдёт smoke check через выбранную board integration; если выбрать `yandex_tracker`, это будет live API-backed `create_task`;
